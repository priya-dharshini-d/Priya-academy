      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-white border-b border-zinc-200" : "bg-transparent"}`}>
        <div className="max-w-[1440px] mx-auto px-4 sm:px-8 md:px-12 flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            <a href="#demo" className="hidden lg:inline-block bg-zinc-900 text-white px-5 py-2 text-xs font-bold uppercase tracking-widest hover:bg-zinc-700 transition-colors">Book Demo</a>
      ...
      <div className="max-w-[1280px] mx-auto px-8 w-full flex flex-col items-center relative z-10">
        ...
        <div className="grid grid-cols-2 gap-4 mt-12 w-full max-w-lg sm:hidden relative z-20 px-4">
          {[
            ...
            { text: "Handwritten Notes for All Chapters", icon: "draw", bg: "bg-[#e14f6b]", anim: "float-3" },
            { text: "Live Doubt Solving Support", icon: "forum", bg: "bg-[#3b82f6]", anim: "float-4" },
            { text: "Mind Maps for All Chapters", icon: "hub", bg: "bg-[#10b981]", anim: "float-2" },
            { text: "Comprehensive Study Materials", icon: "menu_book", bg: "bg-[#f59e0b]", anim: "float-1" }
          ].map((card) => (
            <div key={card.text} className={`${card.bg} ${card.anim} text-white p-4 rounded-2xl shadow-lg flex flex-col justify-center min-h-[110px] hover:scale-105 transition-all duration-300`}>
              <span className="material-symbols-outlined mb-1.5 block text-2xl">{card.icon}</span>
              <div className="font-bold text-[10px] leading-tight">{card.text}</div>
            </div>
          ))}
        </div>
